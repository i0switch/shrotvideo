...
このアプリケーションは、X、TikTok、YouTubeなどのSNSアカウントを監視し、その内容から自動でショート動画を生成・変換するデスクトップツールです。

## 機能概要

- **XのRT監視 → スクショ → 背景動画合成 → ショート動画自動生成/保存**
- **TikTok/YouTube のショート監視 → 転載向けフォーマットへ変換/保存**
...
- **Windows**: `%USERPROFILE%\AppData\Roaming\ShortVideo-Genius\logs`
- **macOS**: `~/Library/Logs/ShortVideo-Genius`

---

## 補足: Playwright ブラウザの同梱と実行

X のスクリーンショット取得には Playwright を利用します。本アプリは `playwright-core` とローカルブラウザ (`node_modules/playwright-core/.local-browsers`) をパッケージに同梱し、実行時に `PLAYWRIGHT_BROWSERS_PATH=0` を設定して同梱ブラウザを強制利用します。これにより、ユーザー環境のキャッシュや後続ダウンロードに依存せず、パッケージ版でも安定動作します。