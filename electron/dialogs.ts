...
ipcMain.handle('files.pickFolder', async (e, key: string) => {
  try {
    ...
    if (win && !win.isDestroyed()) {
      result = await dialog.showOpenDialog(win, opts as any) as unknown as { canceled: boolean; filePaths: string[] };
    } else {
      result = await dialog.showOpenDialog(opts as any) as unknown as { canceled: boolean; filePaths: string[] };
    }
