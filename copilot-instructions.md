...
1. **ログイン**（X/TikTok/YouTube のうち最小 1、可能なら複数）
...
export function createLoginWindow(platform: keyof typeof LOGIN_URL) {
  ...
  const okDomain = /x\.com|tiktok\.com|youtube\.com/.test(host);
    if (!okDomain) return;
...
contextBridge.exposeInMainWorld('auth', {
  login: (platform: 'x'|'tiktok'|'youtube') => ipcRenderer.invoke('auth.login', platform),
});
```
